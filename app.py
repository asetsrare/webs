...
# ---------------------------------------------------------------------------
# Prompts (unchanged – they are huge, but we keep them)
# ---------------------------------------------------------------------------

Base_prompt = """... (your existing Base_prompt) ..."""
BASEREVIEWER_PROMPT = """... (your existing BASEREVIEWER_PROMPT) ..."""
...
# We no longer use INTERACTIONS_URL – use generateContent endpoint
REVIEW_MODEL = "gemini-2.5-flash"   # fast and reliable
...
def extract_gemini_output(result):
    """Extract text from either interactions or generateContent response."""
    # Try interactions-style first
    ...
    # Fallback: generateContent style
    try:
...
def call_gemini(model, prompt, api_key, max_retries=2, timeout=(10, 600)):
    """
    Call Gemini's generateContent endpoint with retries on timeout.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 200:
                return resp
            # If we get a 4xx/5xx that isn't a timeout, abort retry
            ...
            break
    # If all retries fail, raise an exception so the outer handler can catch it
...
def generate():
    try:
        ...
        # --- Generation step (using generateContent) ---
        ...
        # --- Review stage (always runs, using gemini-2.5-flash) ---
        review_result = None
